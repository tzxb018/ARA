...
namespace ARA.Droid
{
    ...
    public class O_IFR_Day_XC_1Departure : FragmentActivity, OnFragmentInteractionListener
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            var mFrg1 = new O_IFR_Day_XC1();
            var mFrg2 = new O_IFR_Day_XC2();
            ...
            sc.riskShow(txtRisk, txtRiskNum, "Departure Airfield Risk", departureRisk, 8, 10);
            ...
            btnNext.Click += (s, e) =>
            {
                if (mCurrent.Equals(mFrg2)) //determining question
                {
                    if (departureRisk > 9)
                    {
                        sc.alertShow("Departure Airfield Risk", this);
                    }
                    else
                    {
        ...
        public void onFragmentInteraction(int riskOut, int riskOut2, int r3)
        {
            ...
            try
            {
                var txtRisk = FindViewById<TextView>(Resource.Id.txtRiskFragment);
                var txtRiskNum = FindViewById<TextView>(Resource.Id.txtRiskNumFragment);
                departureRisk = O_IFR_Day_XC1.risk1 + O_IFR_Day_XC1.risk2 + O_IFR_Day_XC1.risk3 + O_IFR_Day_XC2.risk4 + O_IFR_Day_XC2.risk5;
                sc.riskShow(txtRisk, txtRiskNum, "Alternate Risk", departureRisk, 8, 10);
